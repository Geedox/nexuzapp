...
export class GameTokenManager {
  ...
  PACKAGE_ID: string;
  GAME_TOKEN_STORE_ID: string;
  TOKEN_POLICY_ID: string;
  ADMIN_CAP_ID: string;
  GAME_TOKEN_TYPE: string;
  USDC_TYPE: string;
  USDT_TYPE: string;
  USD_TO_GT_RATE: number;
  PLATFORM_FEE_RATE: number;
  GT_DECIMALS: number;
  USDC_DECIMALS: number;
  USDT_DECIMALS: number;
  constructor(suiClient: SuiClient, network: "testnet" | "mainnet") {
    this.suiClient = suiClient;
    this.network = network;

    // Your deployed contract addresses
    this.PACKAGE_ID =
      "0x28368dd22fdcfd68639d6ef692b666b05c66b4cb601598c6df1163d09769a161";
    this.GAME_TOKEN_STORE_ID =
      "0x361231730e99ca459be4789d58c4dc772fc8d6819de536a8de4447627009eb4f";
    this.TOKEN_POLICY_ID =
      "0x1f6789b62e0a7465aa56c6c9efd3284c97c3d04d438bf549ddb96b02945596a8";
    this.ADMIN_CAP_ID =
      "0x8e85f98102940f4fafe142f563cbfff596768498ae8cb6d17b68f43113059297";
  ...
  async mintGTWithUSDC(walletKeyPair, gtAmountWanted) {
    try {
      ...
      const { usdEquivalent, platformFee, totalPayment } =
        this.calculatePaymentRequired(gtAmountWanted);
      console.log(
        `💰 Want ${gtAmountWanted} GT: Pay $${totalPayment} (${usdEquivalent} + ${platformFee} fee)`
      );
      ...
      const totalPaymentSmallest = Math.floor(
        totalPayment * Math.pow(10, this.USDC_DECIMALS)
      );
      const gtAmountSmallest = Math.floor(
        gtAmountWanted * Math.pow(10, this.GT_DECIMALS)
      );
      ...
      if (totalBalance < totalPaymentSmallest) {
        throw new Error(
          `Insufficient USDC. Need $${totalPayment}, have $${
  ...
  async mintGTWithUSDT(walletKeyPair, gtAmountWanted) {
    try {
      ...
      // Convert to smallest units
      const totalPaymentSmallest = Math.floor(
        totalPayment * Math.pow(10, this.USDT_DECIMALS)
      );
      const gtAmountSmallest = Math.floor(
        gtAmountWanted * Math.pow(10, this.GT_DECIMALS)
      );
      ...
      if (totalBalance < totalPaymentSmallest) {
        throw new Error(
          `Insufficient USDT. Need $${totalPayment}, have $${
  ...
  async burnGTForUSDC(walletKeyPair, gtAmount) {
    try {
      ...
      const { usdEquivalent, platformFee, usdReceived } =
        this.calculateUSDReceived(gtAmount);
      console.log(
        `💸 Sell ${gtAmount} GT: Get $${usdReceived} (${usdEquivalent} - ${platformFee} fee)`
      );
      ...
      const usdcCoin = txb.moveCall({
        ...
        arguments: [txb.object(this.GAME_TOKEN_STORE_ID), burnCoin],
      });
  ...
  async withdrawFees(walletKeyPair, currency, amount) {
    try {
      ...
      const targetFunction =
        currency === "USDC" ? "withdraw_usdc_fees" : "withdraw_usdt_fees";
      const decimals =
        currency === "USDC" ? this.USDC_DECIMALS : this.USDT_DECIMALS;
    ...
    try {
      ...
      return allEvents.map((event) => {
        ...
        return {
          ...
          currency:
            event.parsedJson?.payment_currency ||
            event.parsedJson?.received_currency ||
            "Unknown",
          gtAmount:
            Number(event.parsedJson?.gt_amount || 0) /
            Math.pow(10, this.GT_DECIMALS),
          usdEquivalent:
            Number(event.parsedJson?.usd_equivalent || 0) / Math.pow(10, 6),
          platformFee:
            Number(event.parsedJson?.platform_fee || 0) / Math.pow(10, 6),
          totalPayment:
            Number(event.parsedJson?.total_payment || 0) / Math.pow(10, 6),
          usdReceived:
            Number(event.parsedJson?.usd_received || 0) / Math.pow(10, 6),
        };
      });
    } catch (error) {
...
export const gameTokenUtils = {
  ...
  calculatePaymentRequired: (gtAmount) => {
    ...
    return {
      ...
      platformFee,
      totalPayment,
      breakdown: `Want ${gtAmount} GT → Pay $${totalPayment.toFixed(
        4
      )} ($${usdEquivalent} + $${platformFee.toFixed(4)} fee)`,
    };
  },
  ...
  calculateUSDReceived: (gtAmount) => {
    ...
    return {
      ...
      platformFee,
      usdReceived,
      breakdown: `Sell ${gtAmount} GT → Get $${usdReceived.toFixed(
        4
      )} ($${usdEquivalent} - $${platformFee.toFixed(4)} fee)`,
    };
  },
