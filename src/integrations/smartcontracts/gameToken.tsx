...
export class GameTokenManager {
  ...
  private PACKAGE_ID =
    "0x635b49907d2463300968b1bc13388ccd9c5a0302b1ef7be065d3eba8b6616da0";
  private GAME_TOKEN_STORE_ID =
    "0xb7ba6d465a0536cf71ade84eb0a54d444e0c7e1486cc0c18de567834581fa14e";
  private TOKEN_POLICY_ID =
    "0x1342d88b9688b0deafc977fadaf683aadedab2b4e337c3a57ec99c42004e0f9c";
  private ADMIN_CAP_ID =
    "0x6f98813f138929a405eb03044a37dfafcaec19baad38ca8a35de9b18b1798a3b";
  public SUI_TYPE = "0x2::sui::SUI";
  ...
  async buyGameTokensWithSui(walletKeyPair, suiAmount) {
    try {
      ...
      const requiredAmountMist = Math.floor(
        suiAmount * Math.pow(10, this.SUI_DECIMALS)
      );
      ...
      const gtCoin = txb.moveCall({
        ...
        arguments: [txb.object(this.GAME_TOKEN_STORE_ID), paymentCoin],
      });
  ...
  async buyGameTokensWithUsdc(walletKeyPair, usdcAmount) {
    try {
      ...
      const requiredAmountSmallest = Math.floor(
        usdcAmount * Math.pow(10, this.USDC_DECIMALS)
      );
      ...
      const usdcCoin = txb.object(usdcCoins.data[0].coinObjectId);
  ...
  async buyGameTokensWithUsdt(walletKeyPair, usdtAmount) {
    try {
      ...
      const requiredAmountSmallest = Math.floor(
        usdtAmount * Math.pow(10, this.USDT_DECIMALS)
      );
      const totalBalance = usdtCoins.data.reduce(
        (sum, coin) => sum + Number(coin.balance),
        0
      );
      ...
      const usdtCoin = txb.object(usdtCoins.data[0].coinObjectId);
      if (usdtCoins.data.length > 1) {
        const otherCoins = usdtCoins.data
          .slice(1)
          .map((coin) => txb.object(coin.coinObjectId));
  ...
  async sellGameTokensForSui(walletKeyPair, gtAmount) {
    try {
      ...
      const gtAmountSmallest = Math.floor(
        gtAmount * Math.pow(10, this.GT_DECIMALS)
      );

      // Merge GT coins if needed and split the required amount
      const gtCoin = txb.object(gtCoins[0]);
      if (gtCoins.length > 1) {
      ...
      const suiCoin = txb.moveCall({
        ...
        arguments: [txb.object(this.GAME_TOKEN_STORE_ID), sellCoin],
      });
  ...
  async getGameTokenBalance(address) {
    try {
      ...
      const formattedBalance =
        Number(balance.totalBalance) / Math.pow(10, this.GT_DECIMALS);
      ...
      try {
        ...
        // Filter for GT coins
        const gtCoins = allCoins.data.filter(
          (coin) => coin.coinType === this.GAME_TOKEN_TYPE
        );
        console.log("🎮 GT coins found:", gtCoins);

        if (gtCoins.length > 0) {
          const totalBalance = gtCoins.reduce(
            (sum, coin) => sum + Number(coin.balance),
            0
          );
          const formattedBalance =
            totalBalance / Math.pow(10, this.GT_DECIMALS);
  ...
  async spendForRoomEntry(walletKeyPair, gtAmount, roomId) {
    try {
      ...
      txb.moveCall({
        ...
        arguments: [txb.object(this.TOKEN_POLICY_ID), convertRequest],
      });
  ...
  async addReserves(walletKeyPair, currency, amount) {
    try {
      ...
      const coin = txb.object(coins.data[0].coinObjectId);
  ...
  async mintReward(walletKeyPair, amount, recipientAddress) {
    try {
      ...
      const amountSmallest = Math.floor(
        amount * Math.pow(10, this.GT_DECIMALS)
      );
  ...
  async getStoreStats() {
    try {
      ...
      if (
        ...
        const fields = result.data.content.fields as MoveStruct;
        return {
          totalMinted:
            Number(fields.total_minted) / Math.pow(10, this.GT_DECIMALS),
          totalBurned:
            Number(fields.total_burned) / Math.pow(10, this.GT_DECIMALS),
  ...
  async getReserveBalances() {
    try {
      ...
      if (
        ...
        return {
          sui:
            Number(fields.sui_reserves?.fields?.value || 0) /
            Math.pow(10, this.SUI_DECIMALS),
          usdc:
            Number(fields.usdc_reserves?.fields?.value || 0) /
            Math.pow(10, this.USDC_DECIMALS),
          usdt:
            Number(fields.usdt_reserves?.fields?.value || 0) /
            Math.pow(10, this.USDT_DECIMALS),
        };
      }
  ...
  async getTokenPurchaseHistory(limit = 20) {
    try {
      ...
      return allEvents.map((event) => {
        ...
        const res = event.parsedJson as any;
        return {
          digest: event.id.txDigest,
          timestamp: event.timestampMs,
          type: isPurchase ? "purchase" : "sale",
          user: res?.buyer || res?.seller || "Unknown",
          currency:
            res?.payment_currency || res?.received_currency || "Unknown",
          amount: isPurchase
            ? Number(res?.payment_amount || 0)
            : Number(res?.received_amount || 0),
          gtAmount: isPurchase
            ? Number(res?.gt_received || 0) / Math.pow(10, this.GT_DECIMALS)
            : Number(res?.gt_amount || 0) / Math.pow(10, this.GT_DECIMALS),
          exchangeRate: Number(res?.exchange_rate || 0),
          // Format for wallet display
          paymentAmount: isPurchase ? Number(res?.payment_amount || 0) : 0,
          gtReceived: isPurchase ? Number(res?.gt_received || 0) : 0,
        };
      });
    } catch (error) {
...
export const gameTokenUtils = {
  ...
  calculateGTFromCurrency: (currencyAmount, rate = 3100) =>
    Math.floor(currencyAmount * rate), // Default to SUI rate
...
export const initGameTokenManager = (
  suiClient,
