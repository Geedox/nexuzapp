...
const useWinnerCelebration = () => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("showWinCelebration", handleCustomCelebration);
  }, [triggerCelebration]);

  // Check for wins with enhanced error handling
  const checkForWins = useCallback(
    async (source = "manual") => {
      if (!user) {
      ...
      try {
        addDebugLog(`Checking for wins (source: ${source})`, {
          userId: user.id,
        });
        ...
        if (newWin) {
          ...
          const celebration = {
            ...
            playerName:
              user.username || user.email?.split("@")[0] || "Champion",
          ...
          localStorage.setItem(
            "shownWins",
            JSON.stringify([...stored, newWin.room.id])
          );
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            payload.new &&
            payload.new.earnings > 0 &&
            payload.new.final_position
          ) {
...
const WinnerCelebrationDebugPanel = ({
  debugInfo,
  subscriptionStatus,
  onManualCheck,
}) => {
