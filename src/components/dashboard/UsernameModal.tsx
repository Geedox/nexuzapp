...
export const UsernameModal = ({
  ...
  const { checkUsernameAvailability, updateProfile, uploadAvatar, profile } =
    useProfile();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (avatarMode === "preset" && !selectedAvatar) {
      toast({
    ...
    if (avatarMode === "custom" && !customAvatarUrl) {
      toast({
        ...
        description: "Please upload your custom avatar",
        variant: "destructive",
      });
      return;
    }
    ...
    setSaving(true);
    try {
      // Create avatar URL based on mode
      const avatarUrl =
        avatarMode === "preset"
          ? `avatar_${selectedAvatar!.id}_${selectedAvatar!.emoji}`
          : customAvatarUrl!;
  ...
  return (
    <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent
        className="sm:max-w-[700px] bg-gradient-to-br from-background via-card to-secondary/20 border-primary/30"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
              ...
              <div className="grid grid-cols-5 gap-3">
                {GAMING_AVATARS.map((avatar) => (
                  ...
                  >
                    <div
                      className={`
                      w-full aspect-square rounded-xl bg-gradient-to-br ${
                        avatar.color
                      } 
          ...
          {avatarMode === "custom" && (
            <div className="space-y-3">
              ...
              <AvatarUpload
                ...
                onAvatarChange={setCustomAvatarUrl}
          ...
          {/* Preview Section */}
          {username &&
            ((avatarMode === "preset" && selectedAvatar) ||
              (avatarMode === "custom" && customAvatarUrl)) && (
              <div className="bg-black/40 border border-primary/30 rounded-lg p-4">
                <p className="text-xs font-cyber text-muted-foreground mb-2">
                  PREVIEW
                </p>
                <div className="flex items-center gap-3">
                  {avatarMode === "preset" && selectedAvatar ? (
                    <div
                      className={`w-12 h-12 rounded-lg bg-gradient-to-br ${selectedAvatar.color} flex items-center justify-center text-2xl`}
                    >
