...
interface RoomManagementModalProps {
  open: boolean;
  onClose: () => void;
  room: Database["public"]["Tables"]["chat_rooms"]["Row"];
}

export const RoomManagementModal = ({
  open,
  onClose,
  room,
  ...
  const { user } = useAuth();
  const { getRoomMembers, addUserToRoom, removeUserFromRoom, searchUsers } =
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] bg-gradient-to-br from-background via-card to-secondary/20 border-primary/30 max-h-[80vh] overflow-y-auto">
        ...
        <div className="mt-4">
          {activeTab === "members" && (
            <div className="space-y-4">
              <h3 className="font-cyber font-bold text-primary">
                Room Members
              </h3>
        ...
        <div className="flex justify-end mt-6">
          <Button onClick={onClose} variant="outline" className="font-cyber">
            Close
          </Button>
        </div>
      </DialogContent>
