...
const CommunityPage = ({ supabase }: CommunityPageProps) => {
  const [activeView, setActiveView] = useState<"community" | "chat">(
    "community"
  );
  const [selectedRoom, setSelectedRoom] = useState<ChatRoom | Friend | null>(
    null
  );
  const [chatType, setChatType] = useState<"room" | "individual">("room");
  const [activeTab, setActiveTab] = useState<"rooms" | "friends" | "requests">(
    "rooms"
  );
    ...
    return (
      <div className="h-full animate-fade-in">
        <ChatInterface
          ...
          memberCount={
            chatType === "room"
              ? (selectedRoom as ChatRoom).participant_count
              : undefined
          }
        />
      </div>
          ...
          <div className="bg-black/40 backdrop-blur-lg border border-primary/30 rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-primary/20">
              ...
              <p className="text-sm text-muted-foreground font-cyber">
                Join active gaming communities
              </p>
            </div>
            ...
            <div className="p-6">
              {loadingRooms ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <span className="ml-2 font-cyber text-muted-foreground">
                    Loading rooms...
                  </span>
                </div>
                  ...
                  {filteredRooms.map((room) => (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div className="relative">
                          <div
                            className={`w-12 h-12 rounded-lg bg-gradient-to-r ${room.avatar_color} flex items-center justify-center text-2xl`}
                          >
                            {room.avatar_emoji}
                          </div>
                  ...
                  {filteredFriends.map((friendship) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`text-xs font-cyber px-3 py-1 rounded-full ${
                            status === "online"
