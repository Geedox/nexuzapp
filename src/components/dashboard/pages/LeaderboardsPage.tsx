...
const LeaderboardsPage = () => {
  ...
  const getBadge = (player) => {
    if (activeTab === "global") {
      ...
      if (player.wins >= 5)
        return { name: "ADVANCED", style: "bg-gray-500/20 text-gray-400" };
      return null;
    } else {
      // Game-specific badges based on score
      const score = player.total_score;
      if (activeTab === "11111111-1111-1111-1111-111111111111") {
        // Endless Runner
        if (score >= 5000)
          return { name: "LEGEND", style: "bg-yellow-500/20 text-yellow-400" };
        if (score >= 3000)
      ...
      } else if (activeTab === "22222222-2222-2222-2222-222222222222") {
        // Flappy Bird
        if (score >= 100)
          return { name: "LEGEND", style: "bg-yellow-500/20 text-yellow-400" };
        if (score >= 50)
        ...
        if (score >= 5)
          return { name: "ROOKIE", style: "bg-gray-500/20 text-gray-400" };
      } else if (activeTab === "33333333-3333-3333-3333-333333333333") {
        // Crypto Jump
        if (score >= 100)
          return { name: "LEGEND", style: "bg-yellow-500/20 text-yellow-400" };
        if (score >= 50)
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-4">
                <div
                  className={`text-2xl font-bold font-cyber w-12 text-center ${
                    player.rank <= 3
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3">
                    {badge && (
                      <div
                        className={`text-xs px-3 py-1 rounded-full font-bold font-cyber ${badge.style}`}
                      >
                        {badge.name}
                      </div>
                    )}
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="bg-black/40 backdrop-blur-lg border border-primary/30 rounded-2xl overflow-hidden">
        ...
        <div className="p-6">{renderLeaderboard(getCurrentLeaderboard())}</div>
      </div>

      {/* Legend */}
      <div className="bg-secondary/20 border border-primary/20 rounded-xl p-4">
        <h3 className="font-cyber text-sm font-bold text-primary mb-2">
          Badge Legend
        </h3>
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="bg-yellow-500/20 text-yellow-400 px-2 py-1 rounded-full font-cyber">
            LEGEND
          </span>
          <span className="bg-purple-500/20 text-purple-400 px-2 py-1 rounded-full font-cyber">
            ELITE
          </span>
          <span className="bg-blue-500/20 text-blue-400 px-2 py-1 rounded-full font-cyber">
            PRO
          </span>
          <span className="bg-green-500/20 text-green-400 px-2 py-1 rounded-full font-cyber">
            EXPERT
          </span>
          <span className="bg-gray-500/20 text-gray-400 px-2 py-1 rounded-full font-cyber">
            ROOKIE/ADVANCED
          </span>
        </div>
      </div>
