...
const DashboardRightPanel = () => {
  ...
  const fetchRecentWinners = async () => {
    try {
      ...
      const winners: RecentWinner[] = (data || []).map(transaction => {
        const timeAgo = getTimeAgo(new Date(transaction.created_at));
        const userName = transaction.user?.display_name || transaction.user?.username || 'Anonymous';
        const gameName = transaction.room?.game?.name || transaction.room?.name || 'Unknown Game';
  ...
  const calculateLiveStats = async () => {
    try {
      ...
      const prizePoolToday = (todayWinnings || [])
  ...
  const getBadge = (wins: number) => {
    if (wins >= 100) return { name: 'LEGEND', style: 'bg-yellow-500/20 text-yellow-400' };
    if (wins >= 50) return { name: 'ELITE', style: 'bg-purple-500/20 text-purple-400' };
    if (wins >= 25) return { name: 'PRO', style: 'bg-blue-500/20 text-blue-400' };
    if (wins >= 10) return { name: 'EXPERT', style: 'bg-green-500/20 text-green-400' };
  ...
  const topGamers = globalLeaderboard.slice(0, 5).map((player, index) => ({
    rank: index + 1,
    name: player.user?.display_name || player.user?.username || player.display_name || player.username || 'Anonymous',
    score: player.total_wins || player.wins || 0,
    avatar: getAvatar(index + 1),
    badge: getBadge(player.total_wins || player.wins || 0).name,
    badgeStyle: getBadge(player.total_wins || player.wins || 0).style
  }));
      ...
      <div className="p-3 lg:p-6 space-y-6 lg:space-y-8">
        ...
        <div className="animate-slide-up">
          ...
          <div className="space-y-2 lg:space-y-3">
            {topGamers.length > 0 ? topGamers.map((gamer, index) => (
              <div 
                key={index}
              ...
              >
                <div className="flex items-center space-x-2 lg:space-x-3">
                  ...
                  <div>
                    ...
                    <div className={`text-xs px-1 lg:px-2 py-1 rounded-full font-bold inline-block font-cyber ${gamer.badgeStyle}`}>
                      {gamer.badge}
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-cyber text-xs lg:text-sm text-primary">{gamer.score} wins</div>
                </div>
              </div>
            )) : (
              <div className="text-center py-4">
                <p className="text-muted-foreground text-sm">No players yet</p>
                <p className="text-xs text-muted-foreground">Start playing to appear here!</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2 lg:space-y-3">
            {recentWinners.length > 0 ? recentWinners.map((winner, index) => (
              ...
              <div className="text-center py-4">
                ...
                <p className="text-xs text-muted-foreground">Be the first to win!</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="animate-slide-up" style={{ animationDelay: '0.4s' }}>
          ...
          <div className="space-y-3 lg:space-y-4">
            <div className="bg-black/40 backdrop-blur-lg border border-primary/30 rounded-lg p-2 lg:p-4">
              ...
              <div className="w-full bg-secondary/20 rounded-full h-1 lg:h-2">
                <div 
                  ...
                  style={{ width: `${Math.min(100, (liveStats.activePlayers / 100) * 100)}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-black/40 backdrop-blur-lg border border-accent/30 rounded-lg p-2 lg:p-4">
              <div className="flex justify-between items-center mb-1 lg:mb-2">
                <span className="text-xs lg:text-sm text-muted-foreground font-cyber">Live Rooms</span>
              ...
              <div className="w-full bg-secondary/20 rounded-full h-1 lg:h-2">
                <div 
                  ...
                  style={{ width: `${Math.min(100, (liveStats.liveRooms / 50) * 100)}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-black/40 backdrop-blur-lg border border-green-500/30 rounded-lg p-2 lg:p-4">
              <div className="flex justify-between items-center mb-1 lg:mb-2">
                ...
                <span className="text-sm lg:text-lg font-bold text-green-400 font-cyber">
                  ${liveStats.prizePoolToday.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
              <div className="w-full bg-secondary/20 rounded-full h-1 lg:h-2">
                <div 
                  ...
                  style={{ width: `${Math.min(100, (liveStats.prizePoolToday / 1000) * 100)}%` }}
                ></div>
              </div>
            </div>
          </div>
