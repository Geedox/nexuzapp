  ...
  const fetchRecentWinners = async () => {
    try {
      ...
      const winners: RecentWinner[] = (data || []).map((winner: any) => {
        ...
        const userName = winner.participant?.user?.display_name || winner.participant?.user?.username || 'Anonymous';
        const gameName = winner.participant?.room?.game?.name || winner.participant?.room?.name || 'Unknown Game';
        ...
        <div className="animate-slide-up">
          ...
          <div className="space-y-2 lg:space-y-3">
            {topGamers.length > 0 ? topGamers.map((gamer, index) => (
              <div
                key={index}
                className={`flex items-center justify-between p-2 lg:p-3 rounded-lg border transition-all duration-300 hover:scale-105 cursor-pointer ${gamer.rank === 1
              ...
              >
                <div className="flex items-center space-x-2 lg:space-x-3">
                  <div className={`text-sm lg:text-lg font-bold font-cyber ${gamer.rank <= 3 ? 'text-accent' : 'text-muted-foreground'
                    }`}>
                    #{gamer.rank}
                  </div>
