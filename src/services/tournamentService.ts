...
import { GameRoom as OnChainGameRoom, TransferCurrency } from "../integrations/smartcontracts/gameRoom";
...
class TournamentService {
  ...
  async submitWinnersToSmartContractAndDistributePrizes(winners: Winner[], room: GameRoom): Promise<void> {
    try {
      ...
      const onChainResult = await this.onChainGameRoom.completeGame({
        ...
        winnerAddresses,
        scores,
        currency: room.currency as TransferCurrency
      })
  ...
  async completeMatch(matchId: string, winnerId: string, roomName: string, scores?: Record<string, number>): Promise<TournamentMatch> {
    try {
      ...
      else this.updateRoundRobinStandings(match as TournamentMatch)
  ...
  private async updateRoundRobinStandings(match: TournamentMatch): Promise<void> {
    try {
