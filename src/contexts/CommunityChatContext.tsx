...
const CommunityChatContext = createContext<
  CommunityChatContextType | undefined
>(undefined);
...
export const useCommunityChatContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useCommunityChatContext must be used within a CommunityChatProvider"
    );
  }
  return context;
};
...
export const CommunityChatProvider: React.FC<CommunityChatProviderProps> = ({
  children,
}) => {
  const { user } = useAuth();
  ...
  const sendFriendRequest = async (username: string) => {
    ...
    try {
      ...
      const { data: existingRequest } = await supabase
        ...
        .select("id")
        .or(
          `and(requester_id.eq.${user.id},addressee_id.eq.${userData.id}),and(requester_id.eq.${userData.id},addressee_id.eq.${user.id})`
        )
        .single();
  ...
  }, [user, loadChatRooms, loadFriends, updateOnlineStatus]);
