 ...
 * 
 * Purpose: Verify that when a sponsored room is cancelled, 
 * the creator receives the full prize pool back.
 */
async function testSponsoredRoomCancellation(): Promise<void> {
  const testName = "Sponsored Room Cancellation";
  const testNumber = 4;
  ...
  try {
    ...
    // Step 3: Cancel sponsored room
    console.log("\nStep 3: Cancelling sponsored room...");
    console.log("Cancelling sponsored room...");
    ...
    console.log(`Sponsor amount refunded: ${formatUSDC(sponsorAmount)} USDC`);
    ...
    // Key 1 (Creator/Sponsor): Initial - sponsor amount + sponsor amount = Initial (full refund)
...
export { testSponsoredRoomCancellation }; 