...
async function transfer(): Promise<void> {
  ...
  try {
    ...
    const { key1, key2, adminKey } = validateEnvironment();
    ...
    const adminKeypair = createKeypair(adminKey, "Admin");
    ...
    const initialBalances = await getAllBalances(client, [keypair1, adminKeypair]);
    ...
    //1: Transfer SUI
    console.log("\n=== SUI Transfer ===");
    const suiTransferAmount = 0.15; // 0.5 SUI

    // if (readiness1.sui.sufficient) {
    console.log(`Transferring ${suiTransferAmount} SUI from Key 1 to Key 2...`);

    const suiResult = await transferSUI(client, keypair1.keypair, adminKeypair.address, suiTransferAmount);
    ...
    // Final balance check
    console.log("\n=== Final Balances ===");
    const finalBalances = await getAllBalances(client, [keypair1, adminKeypair]);
